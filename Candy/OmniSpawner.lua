...
local function check_files()
	if OmniVehicleSpawner then
		ui.notify_above_map("Omni Spawner 已加载完成", "Omni Spawner", 140)
		return
	end
...
local function GetFiles()
	...
	vehFeature = vehFeature or menu.add_feature("载具", "parent", rootId)
	iniVehFeature = iniVehFeature or menu.add_feature("INI格式载具", "parent", vehFeature.id)
	xmlVehFeature = xmlVehFeature or menu.add_feature("XML格式载具", "parent", vehFeature.id)
	vehFeature.hidden = true
	iniVehFeature.hidden = true
	xmlVehFeature.hidden = true
	mapFeature = mapFeature or menu.add_feature("地图", "parent", rootId)
	iniMapFeature = iniMapFeature or menu.add_feature("INI格式地图", "parent", mapFeature.id)
	xmlMapFeature = xmlMapFeature or menu.add_feature("XML格式地图", "parent", mapFeature.id)
...
if not GetFiles() then
	ui.notify_above_map("无载具模组，请添加！", Settings.Settings.ModTitle, Settings.Settings.NotifColour)
	return
end
...
SpawnInVehicleFeature = menu.add_feature("生成时传送到载具", "toggle", vehFeature.id)
...
NoCollisionFeature = menu.add_feature("附加物体无碰撞", "toggle", vehFeature.id)
...
menu.add_feature("保存设置", "action", rootId, function()
	...
	ui.notify_above_map("保存设置", Settings.Settings.ModTitle, Settings.Settings.NotifColour)
	return HANDLER_POP
end).threaded = false

ui.notify_above_map("Omni Spawner 已经加载", Settings.Settings.ModTitle, Settings.Settings.NotifColour)
